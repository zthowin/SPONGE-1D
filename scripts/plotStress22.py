#--------------------------------------------------------------------------------------------------
# Plotting script for off-axial stress at single depth(s) for multiple simulations.
#
...
def main(a_TimeDict, a_StressDict, params):
  ...
  if not os.path.isfile(simA_stress_prime_fname) or not os.path.isfile(simA_tau_fname):
  ...
  if params.simB_Dir is not None:
    ...
    if not os.path.isfile(simB_stress_prime_fname) or not os.path.isfile(simB_tau_fname):
  ...
  if params.simC_Dir is not None:
    ...
    if not os.path.isfile(simC_stress_prime_fname) or not os.path.isfile(simC_tau_fname):
  ...
  if params.simD_Dir is not None:
    ...
    if not os.path.isfile(simD_stress_prime_fname) or not os.path.isfile(simD_tau_fname):
  ...
  if params.simE_Dir is not None:
    ...
    if not os.path.isfile(simE_stress_prime_fname) or not os.path.isfile(simE_tau_fname):
  ...
  # Simulation A
  #
  simA_stress      = np.load(simA_stress_prime_fname) - np.load(simA_tau_fname)
  ...
  #
  if params.simB_Dir is not None:
    simB_stress      = np.load(simB_stress_prime_fname) - np.load(simB_tau_fname)
    ...
    if params.simB_isPython:
      ...
      simB_Gauss  = getGaussPoint(params, simB_coordsGauss, params.simC_probe_1)
  ...
  #
  if params.simC_Dir is not None:
    simC_stress      = np.load(simC_stress_prime_fname) - np.load(simC_tau_fname)
  ...
  #
  if params.simD_Dir is not None:
    simD_stress      = np.load(simD_stress_prime_fname) - np.load(simD_tau_fname)
  ...
  #
  if params.simE_Dir is not None:
    simE_stress      = np.load(simE_stress_fname) - np.load(simE_tau_fname)
  ...
  #-------------------------
  # Transverse solid stress.
  #-------------------------
  if params.simA_isPython:
    ax1.plot(simA_tsolve[::params.simA_Skip]*params.timeScaling, simA_stress[::params.simA_Skip,simA_Gauss[0], simA_Gauss[1]]*params.stressScaling, params.simA_Linestyle_Alpha, color=params.simA_Color_Alpha, fillstyle=params.simA_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simA_probe_1) + r'\text{m}' + r',t)$, ' + params.simA_Title)

  elif params.simA_isDYNA:
    ax1.plot(simA_tsolve[::params.simA_Skip]*params.timeScaling, simA_stress[::params.simA_Skip,simA_Gauss]*params.stressScaling, params.simA_Linestyle_Alpha, color=params.simA_Color_Alpha, fillstyle=params.simA_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simA_probe_1) + r'\text{m}' + r',t)$, ' + params.simA_Title)

  if params.simB_Dir is not None:
    if params.simB_isPython:
      ax1.plot(simB_tsolve[::params.simB_Skip]*params.timeScaling, simB_stress[::params.simB_Skip,simB_Gauss[0], simB_Gauss[1]]*params.stressScaling, params.simB_Linestyle_Alpha, color=params.simB_Color_Alpha, fillstyle=params.simB_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simB_probe_1) + r'\text{m}' + r',t)$, ' + params.simB_Title)

    elif params.simB_isDYNA:
      ax1.plot(simB_tsolve[::params.simB_Skip]*params.timeScaling, simB_stress[::params.simB_Skip,simB_Gauss]*params.stressScaling, params.simB_Linestyle_Alpha, color=params.simB_Color_Alpha, fillstyle=params.simB_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simB_probe_1) + r'\text{m}' + r',t)$, ' + params.simB_Title)
    
  if params.simC_Dir is not None:
    if params.simC_isPython:
      ax1.plot(simC_tsolve[::params.simC_Skip]*params.timeScaling, simC_stress[::params.simC_Skip,simC_Gauss[0], simC_Gauss[1]]*params.stressScaling, params.simC_Linestyle_Alpha, color=params.simC_Color_Alpha, fillstyle=params.simC_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simC_probe_1) + r'\text{m}' + r',t)$, ' + params.simC_Title)

    elif params.simC_isDYNA:
      ax1.plot(simC_tsolve[::params.simC_Skip]*params.timeScaling, simC_stress[::params.simC_Skip,simC_Gauss]*params.stressScaling, params.simC_Linestyle_Alpha, color=params.simC_Color_Alpha, fillstyle=params.simC_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simC_probe_1) + r'\text{m}' + r',t)$, ' + params.simC_Title)

  if params.simD_Dir is not None:
    if params.simD_isPython:
      ax1.plot(simD_tsolve[::params.simD_Skip]*params.timeScaling, simD_stress[::params.simD_Skip,simD_Gauss[0], simD_Gauss[1]]*params.stressScaling, params.simD_Linestyle_Alpha, color=params.simD_Color_Alpha, fillstyle=params.simD_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simD_probe_1) + r'\text{m}' + r',t)$, ' + params.simD_Title)

    elif params.simD_isDYNA:
      ax1.plot(simD_tsolve[::params.simD_Skip]*params.timeScaling, simD_stress[::params.simD_Skip,simD_Gauss]*params.stressScaling, params.simD_Linestyle_Alpha, color=params.simD_Color_Alpha, fillstyle=params.simD_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simD_probe_1) + r'\text{m}' + r',t)$, ' + params.simD_Title)
    
  if params.simE_Dir is not None:
    if params.simE_isPython:
      ax1.plot(simE_tsolve[::params.simE_Skip]*params.timeScaling, simE_stress[::params.simE_Skip,simE_Gauss[0], simE_Gauss[1]]*params.stressScaling, params.simE_Linestyle_Alpha, color=params.simE_Color_Alpha, fillstyle=params.simE_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simE_probe_1) + r'\text{m}' + r',t)$, ' + params.simE_Title)

    elif params.simE_isDYNA:
      ax1.plot(simE_tsolve[::params.simE_Skip]*params.timeScaling, simE_stress[::params.simE_Skip,simE_Gauss]*params.stressScaling, params.simE_Linestyle_Alpha, color=params.simE_Color_Alpha, fillstyle=params.simE_fillstyle, label=r'$\sigma_{22(E)}^\rs(X(\xi) \approx ' + str(params.simE_probe_1) + r'\text{m}' + r',t)$, ' + params.simE_Title)
