#--------------------------------------------------------------------------------------------------
# Plotting script for animation of pressure contour(s) vs. volume contour(s) 
...
def main(a_TimeDict, a_DispDict, params):
  ...
  try:
    for timeIndex in range(params.startID, params.stopID, params.simA_Skip):
      ...
      # Pore fluid volume.
      #-------------------
      ax2.plot(simA_coordsGauss, 1/simA_rhofr[timeIndex,:], params.simA_Linestyle_Alpha, color=params.simA_Color_Alpha, fillstyle=params.simA_fillstyle)

      if params.simB_Dir is not None:
        ax2.plot(simB_coordsGauss, 1/simB_rhofr[timeIndex,:], params.simB_Linestyle_Alpha, color=params.simB_Color_Alpha, fillstyle=params.simB_fillstyle)

      if params.simC_Dir is not None:
        ax2.plot(simC_coordsGauss, 1/simC_rhofr[timeIndex,:], params.simC_Linestyle_Alpha, color=params.simC_Color_Alpha, fillstyle=params.simC_fillstyle)

      if params.simD_Dir is not None:
        ax2.plot(simD_coordsGauss, 1/simD_rhofr[timeIndex,:], params.simD_Linestyle_Alpha, color=params.simD_Color_Alpha, fillstyle=params.simD_fillstyle)

      if params.simE_Dir is not None:
        ax2.plot(simE_coordsGauss, 1/simE_rhofr[timeIndex,:], params.simE_Linestyle_Alpha, color=params.simE_Color_Alpha, fillstyle=params.simE_fillstyle)
      #------------------------
      ...
      if not params.no_labels:
        ...
        ax2.set_ylabel(r'$v^{\rm{fR}}/v_0^{\rm{fR}}$', fontsize=params.yAxisFontSize)
