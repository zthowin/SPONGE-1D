#--------------------------------------------------------------------------------------------------
# Plotting script for animation of hydraulic conductivity vs. Jacobian contour(s).
#
...
#      ax2 = ax1.twinx()
      ...
      #--------------------------------------
      ax1.plot(simA_J[timeIndex,:, simA_khat[timeIndex,:], \
               params.simA_Linestyle_Alpha, color=params.simA_Color_Alpha, fillstyle=params.simA_fillstyle, \
               label=r'$\hat{k}/\hat{k}_0(X(\xi), t\approx$ ' + "{:.2e}".format(simA_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simA_Title)
      
      if params.simB_Dir is not None:
        ax1.plot(simB_coordsGauss, simB_khat[timeIndex,:], \
                 params.simB_Linestyle_Alpha, color=params.simB_Color_Alpha, fillstyle=params.simB_fillstyle, \
                 label=r'$\hat{k}/\hat{k}_0(X(\xi), t\approx$ ' + "{:.2e}".format(simB_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simB_Title)

      if params.simC_Dir is not None:
        ax1.plot(simC_coordsGauss, simC_khat[timeIndex,:], \
                 params.simC_Linestyle_Alpha, color=params.simC_Color_Alpha, fillstyle=params.simC_fillstyle, \
                 label=r'$\hat{k}/\hat{k}_0(X(\xi), t\approx$ ' + "{:.2e}".format(simC_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simC_Title)

      if params.simD_Dir is not None:
        ax1.plot(simD_coordsGauss, simD_khat[timeIndex,:], \
                 params.simD_Linestyle_Alpha, color=params.simD_Color_Alpha, fillstyle=params.simD_fillstyle, \
                 label=r'$\hat{k}/\hat{k}_0(X(\xi), t\approx$ ' + "{:.2e}".format(simD_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simD_Title)

      if params.simE_Dir is not None:
        ax1.plot(simE_coordsGauss, simE_khat[timeIndex,:], \
                 params.simE_Linestyle_Alpha, color=params.simE_Color_Alpha, fillstyle=params.simE_fillstyle, \
                 label=r'$\hat{k}/\hat{k}_0(X(\xi), t\approx$ ' + "{:.2e}".format(simE_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simE_Title)
      #------------------------------
      # Plot Jacobian of deformation.
      #------------------------------
#      ax2.plot(simA_coordsGauss, simA_J[timeIndex,:], \
#               params.simA_Linestyle_Bravo, color=params.simA_Color_Bravo, fillstyle=params.simA_fillstyle, \
#               label=r'$J(X(\xi), t\approx$ ' + "{:.2e}".format(simA_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simA_Title)
#
#      if params.simB_Dir is not None:
#        ax2.plot(simB_coordsGauss, simB_J[timeIndex,:], \
#                 params.simB_Linestyle_Bravo, color=params.simB_Color_Bravo, fillstyle=params.simB_fillstyle, \
#                 label=r'$J(X(\xi), t\approx$ ' + "{:.2e}".format(simB_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simB_Title)
#
#      if params.simC_Dir is not None:
#        ax2.plot(simC_coordsGauss, simC_J[timeIndex,:], \
#                 params.simC_Linestyle_Bravo, color=params.simC_Color_Bravo, fillstyle=params.simC_fillstyle, \
#                 label=r'$J(X(\xi), t\approx$ ' + "{:.2e}".format(simC_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simC_Title)
#
#      if params.simD_Dir is not None:
#        ax2.plot(simD_coordsGauss, simD_J[timeIndex,:], \
#                 params.simD_Linestyle_Bravo, color=params.simD_Color_Bravo, fillstyle=params.simD_fillstyle, \
#                 label=r'$J(X(\xi), t\approx$ ' + "{:.2e}".format(simD_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simD_Title)
#
#      if params.simE_Dir is not None:
#        ax2.plot(simE_coordsGauss, simE_J[timeIndex,:], \
#                 params.simE_Linestyle_Bravo, color=params.simE_Color_Bravo, fillstyle=params.simE_fillstyle, \
#                 label=r'$J(X(\xi), t\approx$ ' + "{:.2e}".format(simE_tsolve[timeIndex]*params.timeScaling) + a_TimeDict[params.timeScaling].split('(')[1] + ', ' + params.simE_Title)
      #------------------------------------
