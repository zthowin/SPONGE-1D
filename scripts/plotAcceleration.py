#--------------------------------------------------------------------------------------------------
# Plotting script for accelerations(s) at single depth(s) for multiple simulations.
#
...
def main(a_TimeDict, params):
  ...
  # Generate file names.
  #---------------------
  simA_acc_fname  = params.simA_Dir + 'acceleration.npy'
  simA_time_fname = params.simA_Dir + 'time.npy'

  if not os.path.isfile(simA_acc_fname):
    sys.exit("-------\nERROR:\n-------\nAcceleration data file not found for Simulation A.")

  if params.simB_Dir is not None:
    simB_acc_fname  = params.simB_Dir + 'acceleration.npy'
    simB_time_fname = params.simB_Dir + 'time.npy'
    if not os.path.isfile(simB_acc_fname):
      sys.exit("-------\nERROR:\n-------\nAcceleration data file not found for Simulation B.")

  if params.simC_Dir is not None:
    simC_acc_fname  = params.simC_Dir + 'acceleration.npy'
    simC_time_fname = params.simC_Dir + 'time.npy'
    if not os.path.isfile(simC_acc_fname):
      sys.exit("-------\nERROR:\n-------\nAcceleration data file not found for Simulation C.")

  if params.simD_Dir is not None:
    simD_acc_fname  = params.simD_Dir + 'acceleration.npy'
    simD_time_fname = params.simD_Dir + 'time.npy'
    if not os.path.isfile(simD_acc_fname):
      sys.exit("-------\nERROR:\n-------\nAcceleration data file not found for Simulation D.")

  if params.simE_Dir is not None:
    simE_acc_fname  = params.simE_Dir + 'acceleration.npy'
    simE_time_fname = params.simE_Dir + 'time.npy'
    if not os.path.isfile(simE_acc_fname):
      sys.exit("-------\nERROR:\n-------\nAcceleration data file not found for Simulation E.")
  ...
  # Simulation A
  #
  simA_Asolve     = np.load(simA_acc_fname)
  ...
  if params.simA_isPython:
    ...
    simA_tsolve = np.linspace(0, simA_simParams.TStop, simA_Asolve.shape[0])
  ...
  #
  if params.simB_Dir is not None:
    simB_Asolve     = np.load(simB_acc_fname)
    ...
    if params.simB_isPython:
      ...
      simB_tsolve = np.linspace(0, simB_simParams.TStop, simB_Asolve.shape[0])
  ...
  #
  if params.simC_Dir is not None:
    simC_Asolve     = np.load(simC_acc_fname)
    ...
    if params.simC_isPython:
      ...
      simC_tsolve = np.linspace(0, simC_simParams.TStop, simC_Asolve.shape[0])
  ...
  #
  if params.simD_Dir is not None:
    simD_Asolve     = np.load(simD_acc_fname)
    ...
    if params.simD_isPython:
      ...
      simD_tsolve = np.linspace(0, simD_simParams.TStop, simD_Asolve.shape[0])
  ...
  #
  if params.simE_Dir is not None:
    simE_Asolve     = np.load(simE_acc_fname)
    ...
    if params.simE_isPython:
      ...
      simE_tsolve = np.linspace(0, simE_simParams.TStop, simE_Asolve.shape[0])
  ...
  #--------------------------
  # Plot solid accelerations.
  #--------------------------
  if params.solidPlot:
    plt.plot(simA_tsolve[::params.simA_Skip]*params.timeScaling, simA_Asolve[::params.simA_Skip,simA_DDOF], params.simA_Linestyle_Alpha, color=params.simA_Color_Alpha, fillstyle=params.simA_fillstyle, label=r'$a(X' + signA + str(params.simA_probe_1) + r'\text{m}' + r',t)$, ' + params.simA_Title)

    if params.simB_Dir is not None:
      plt.plot(simB_tsolve[::params.simB_Skip]*params.timeScaling, simB_Asolve[::params.simB_Skip,simB_DDOF], params.simB_Linestyle_Alpha, color=params.simB_Color_Alpha, fillstyle=params.simB_fillstyle, label=r'$a(X' + signB + str(params.simB_probe_1) + r'\text{m}' + r',t)$, ' + params.simB_Title)
      
    if params.simC_Dir is not None:
      plt.plot(simC_tsolve[::params.simC_Skip]*params.timeScaling, simC_Asolve[::params.simC_Skip,simC_DDOF], params.simC_Linestyle_Alpha, color=params.simC_Color_Alpha, fillstyle=params.simC_fillstyle, label=r'$a(X' + signC + str(params.simC_probe_1) + r'\text{m}' + r',t)$, ' + params.simC_Title)

    if params.simD_Dir is not None:
      plt.plot(simD_tsolve[::params.simD_Skip]*params.timeScaling, simD_Asolve[::params.simD_Skip,simD_DDOF], params.simD_Linestyle_Alpha, color=params.simD_Color_Alpha, fillstyle=params.simD_fillstyle, label=r'$a(X' + signD + str(params.simD_probe_1) + r'\text{m}' + r',t)$, ' + params.simD_Title)
      
    if params.simE_Dir is not None:
      plt.plot(simE_tsolve[::params.simE_Skip]*params.timeScaling, simE_Asolve[::params.simE_Skip,simE_DDOF], params.simE_Linestyle_Alpha, color=params.simE_Color_Alpha, fillstyle=params.simE_fillstyle, label=r'$a(X' + signE + str(params.simE_probe_1) + r'\text{m}' + r',t)$, ' + params.simE_Title)
  #-------------------------------
  # Plot pore fluid accelerations.
  #-------------------------------
  if params.fluidPlot:
    if 'uf' in simA_simParams.Physics:
      plt.plot(simA_tsolve[::params.simA_SkipSecondary]*params.timeScaling, simA_Asolve[::params.simA_SkipSecondary,simA_DFDOF], params.simA_Linestyle_Bravo, color=params.simA_Color_Bravo, fillstyle=params.simA_fillstyle, label=r'$a_\rf(X' + signA + str(params.simA_probe_1) + r'\text{m}' + r',t)$, ' + params.simA_Title)

    if params.simB_Dir is not None:
      if 'uf' in simB_simParams.Physics:
        plt.plot(simB_tsolve[::params.simB_SkipSecondary]*params.timeScaling, simB_Asolve[::params.simB_SkipSecondary,simB_DFDOF], params.simB_Linestyle_Bravo, color=params.simB_Color_Bravo, fillstyle=params.simB_fillstyle, label=r'$a_\rf(X' + signB + str(params.simB_probe_1) + r'\text{m}' + r',t)$, ' + params.simB_Title)
      
    if params.simC_Dir is not None:
      if 'uf' in simC_simParams.Physics:
        plt.plot(simC_tsolve[::params.simC_SkipSecondary]*params.timeScaling, simC_Asolve[::params.simC_SkipSecondary,simC_DFDOF], params.simC_Linestyle_Bravo, color=params.simC_Color_Bravo, fillstyle=params.simC_fillstyle, label=r'$a_\rf(X' + signC + str(params.simC_probe_1) + r'\text{m}'+  r',t)$, ' + params.simC_Title)

    if params.simD_Dir is not None:
      if 'uf' in simD_simParams.Physics:
        plt.plot(simD_tsolve[::params.simD_SkipSecondary]*params.timeScaling, simD_Asolve[::params.simD_SkipSecondary,simD_DFDOF], params.simD_Linestyle_Bravo, color=params.simD_Color_Bravo, fillstyle=params.simD_fillstyle, label=r'$a_\rf(X' + signD + str(params.simD_probe_1) + r'\text{m}' + r',t)$, ' + params.simD_Title)
      
    if params.simE_Dir is not None:
      if 'uf' in simE_simParams.Physics:
        plt.plot(simE_tsolve[::params.simE_SkipSecondary]*params.timeScaling, simE_Asolve[::params.simE_SkipSecondary,simE_DFDOF], params.simE_Linestyle_Bravo, color=params.simE_Color_Bravo, fillstyle=params.simE_fillstyle, label=r'$a_\rf(X' + signE + str(params.simE_probe_1) + r'\text{m}' + r',t)$, ' + params.simE_Title)  

  if not params.no_labels:
    plt.xlabel(r'Time ' + a_TimeDict[params.timeScaling], fontsize=params.xAxisFontSize)
    plt.ylabel(r'Acceleration (m/s$^2$)', fontsize=params.yAxisFontSize)
