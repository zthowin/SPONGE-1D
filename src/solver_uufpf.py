#--------------------------------------------------------------------------------------
# Module to initiate problem parameters and call a time integrator for the u-uf-pf
# poroelastodynamics equations.
#
...
def main(params):
  ...
  #---------------------------------
  if params.integrationScheme == 'Newmark-beta':
    savedData = moduleNR.integrate_Newmark_beta(LM, [GEXT, HEXT], D, V, A, params)
  elif params.integrationScheme == 'RKFNC' or params.integrationScheme == 'RKBS':
    savedData = moduleRK.integrate_uufpf_FO(LM, [GEXT, HEXT], D, V, A, params)
  else:
    sys.exit("-----------------\nINPUT FILE ERROR:\n-----------------\nOther integration schemes not yet implemented for (u-uf-pf).")
