#--------------------------------------------------------------------------------------
# Module to initiate problem parameters and call a time integrator for the u-pf-ts-tf
...
def main(params):
  ...
  if params.integrationScheme == 'RKFNC' or params.integrationScheme == 'RKBS':
    savedData = moduleRK.integrate_upftstf_FO(LM, [GEXT, HEXT, JEXT, KEXT], D, V, A, params)
  elif params.integrationScheme == 'Newmark-beta':
    savedData = moduleNR.integrate_Newmark_beta(LM, [GEXT, HEXT, JEXT, KEXT], D, V, A, params)
  elif params.integrationScheme == 'Trapezoidal':
    savedData = moduleNR.integrate_Trapezoidal(LM, [GEXT, HEXT, JEXT, KEXT], D, V, params)
  else:
    sys.exit("-----------------\nINPUT FILE ERROR:\n-----------------\nUse a variant of Runge-Kutta or implicit Newmark-beta/Trapezoidal time integrators for (u-pf-ts-tf) formulation.")
